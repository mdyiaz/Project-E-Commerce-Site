...
const Baverages = () => {
 ...
 const {data:baverages = [] } = useQuery({
    queryKey:['baverages'],
    queryFn: async () => {
        const res = await fetch('https://e-commerce-server-site.vercel.app/allproducts/Baverages')
        ...
        <div className='px-12'>
            ...
            <div className='flex gap-16'>
                ...
                <div className='grow'>
                 ...
                 {/* Baverages all Products START______________________________________________________ */}


                    <div className=''>
                    {   
                baverages?.map(baverage => {
                    return (
                        <div  key={baverage._id}>

                            <div className="card card-side bg-base-100 shadow-xl">
                                <figure><img src={baverage.picture}alt="Movie" className='h-40'/></figure>
                                <div className="card-body">
                                    <h2 className="card-title text-base  text-slate-700 font-serif">{baverage.name}</h2>
                                    <p className='text-green-600 font-bold text-xs'>{baverage.stock}</p>
                                    ...
                                    <div className="card-actions justify-between ">
                                        <button className="btn btn-success btn-sm text-white ">{baverage.price}</button>


                                    
                                            <Link to="/addtocart"><button className="btn btn-outline btn-primary btn-sm">Add to cart</button></Link> <br />


                                            <Link to={`/checkout/${baverage._id}`}><button className="btn btn-outline btn-accent btn-sm ">Buy</button></Link>
              ...
              {/* Baverages all Products END______________________________________________________ */}
